...
func ListComment(userid, posid int) (listComm Comment, err error) {
	o := orm.NewOrm()
	err = o.Raw("select * from comment where userId = ? and positionId = ? limit 1", userid, posid).QueryRow(&listComm)
	if err != nil {
		return listComm, err
	}
	return listComm, nil
}
func ListCommentPage(pid, page, limit int) (listComm []UserComment) {
	listComm = ListUserComment(pid)
	if cap(listComm) >= limit*page {
		listComm = listComm[page*limit-limit : page*limit]
	}
...
func ListUserComment(pid int) (listComm []UserComment) {
	o := orm.NewOrm()
	if _, err := o.Raw("select commentId,type,content,releaseTime,user.* from comment,user "+
		"where comment.userId = user.userId and positionId = ? order by releaseTime DESC", pid).QueryRows(&listComm); err != nil {
		fmt.Println(err)
		return nil
	}
	return listComm
}
func ListCommentUid(userid int) (listComm []Comment, err error) {
	o := orm.NewOrm()
	_, err = o.Raw("select * from comment where userId = ?", userid).QueryRows(&listComm)
	if err != nil {
		return nil, err
	}
	return listComm, nil
